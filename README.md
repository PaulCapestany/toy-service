...
CI sanity check (post-private): this doc-only change exercises the Tekton -> Quay -> Image Updater flow; no runtime behavior changes.
...
- **GET /healthz:** Check if the service is running (`Cache-Control: no-store` prevents caching).
